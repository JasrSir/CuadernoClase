...
public class AdapterControles_Add extends ArrayAdapter {
    ...
    public AdapterControles_Add(Context context) {
        super(context, R.layout.faltahoy_item);
        for (Estudiante es : AdapterEstudiantes.estudiantesList) {
            if (es.getControl().getObservacion().isEmpty())
                add(es);
        }
    ...
    public View getView(final int position, View convertView, final ViewGroup parent) {
        ...
        switch (((Estudiante) getItem(position)).getFaltasB()) {
            case 0:
            ...
            case 2:
                ...
                break;
            default:
                holderf.falta.setText("");
                holderf.faltaimg.setImageResource(R.mipmap.ic_touch);
                break;
        }
        switch (((Estudiante) getItem(position)).getTrabajoB()) {
            case 0:
            ...
            case 2:
                ...
                break;
            default:
                holderf.trabajo.setText("");
                holderf.trabajoimg.setImageResource(R.mipmap.ic_touch);
                break;

        }
        switch (((Estudiante) getItem(position)).getActitudB()) {
            case 0:
            ...
            case 1:
                ...
                break;
            default:
                holderf.actitud.setText("");
                holderf.actitudimg.setImageResource(R.mipmap.ic_touch);
                break;

        }

        holder.observaciones.requestFocus();
        if (!((Estudiante) getItem(position)).getControl().getObservacion().isEmpty()) {
            holderf.observaciones.setText(((Estudiante) getItem(position)).getControl().getObservacion());
        } else
            holderf.observaciones.setText("");
        ...
        holder.observaciones.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ((Estudiante) getItem(position)).getControl().setObservacion(s.toString());
            }
        ...
        holder.fabControlAdd.setOnClickListener(new View.OnClickListener() {
            ...
            private void guardar(final Object item) {
                ...
                ((Estudiante) item).setControl(control);
